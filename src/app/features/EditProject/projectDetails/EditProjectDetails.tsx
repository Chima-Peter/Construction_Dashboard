      ...
      <section className="pt-4 pb-6 px-4 lg:px-8 rounded-sm w-[100%] flex flex-col gap-4">
         ...
         <div className="flex w-[100%] gap-4 md:gap-0 md:gap-y-4 justify-normal md:justify-between flex-wrap">
            <label htmlFor="name" className="text-xs font-medium gap-0 w-[100%] md:w-[48%] flex flex-col"> Project Name
               <input autoFocus type="text" autoComplete="off" onBlur={handleBlur} value={projectDetails?.name} onChange={handleInput} name="name" id="name" className="w-[100%] capitalize px-4 py-2 border-b-2 border-b-gray-200 outline-none text-[13px] focus:border-black placeholder:text-xs placeholder:text-gray-500 placeholder:normal-case" />
            </label>
            <label htmlFor="manager" className="text-xs font-medium gap-0 w-[100%] md:w-[48%] flex flex-col"> Project Manager
               <input type="text" autoComplete="off" onBlur={handleBlur} value={projectDetails?.manager} onChange={handleInput} name="manager" id="manager" className="capitalize w-[100%] px-4 py-2 border-b-2 border-b-gray-200 outline-none text-[13px] focus:border-black placeholder:text-xs placeholder:text-gray-500 placeholder:normal-case" />
            </label>
         </div>
         <div className="flex w-[100%] gap-y-4 justify-between flex-wrap mt-4">
            <ul className="flex flex-col gap-1 list-inside list-decimal w-[100%] lg:w-[48%]">
               ...
               {
                  projectDetails.milestones.map((_item: any, index: any) => (
                     <li key={index} className="flex gap-4 items-center">
                        <input type="text" name="text" autoComplete="off" onChange={(event) => handleMilestone(event, index)} onBlur={handleBlur} value={projectDetails.milestones[index]} className="w-[100%] px-4 py-2 border-b-2 border-b-gray-200 outline-none text-[13px] focus:border-black placeholder:text-xs placeholder:text-gray-500 placeholder:normal-case" placeholder={`Milestone ${index + 1}`}/>
                        {
                           (projectDetails.milestones.length > 1) && <MdCancel className="text-red-600 w-4 h-4 cursor-pointer" onClick={() => removeLastMilestone(index)} />
                        }
                     </li>
                  ))
               }
            ...
            <ul className="flex flex-col gap-1 list-inside list-decimal w-[100%] lg:w-[48%]">
               ...
               {
                  projectDetails.keyDetails.map((_item: any, index: any) => (
                     <li key={index} className="flex gap-4 items-center">
                        <input type="text" name="text" autoComplete="off" onChange={(event) => handleKeyDetail(event, index)} onBlur={handleBlur} value={projectDetails.keyDetails[index]} className="w-[100%] px-4 py-2 border-b-2 border-b-gray-200 outline-none text-[13px] focus:border-black placeholder:text-xs placeholder:text-gray-500 placeholder:normal-case" placeholder={`Detail ${index + 1}`}/>
                        {
                           (projectDetails.keyDetails.length > 1) && <MdCancel className="text-red-600 w-4 h-4 cursor-pointer" onClick={() => removeLastKeyDetail(index)} />
                        }
                     </li>
                  ))
               }
